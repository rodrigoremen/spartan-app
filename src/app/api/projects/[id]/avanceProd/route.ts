	...
	try {
		const newAvances = await prisma.$transaction(
			data.tarea.map((tarea: any) => {
				return prisma.avanceProduccion.upsert({
					where: { id: tarea.id || '' }, // Aquí aseguramos que si id es undefined o '', no causará un error
					update: {
						titulo: tarea.titulo,
...
export async function GET(
	...
	const avances = await prisma.avanceProduccion.findMany({
		where: {
			projectId: parseInt(params.id),
		},
	});
...
export async function DELETE(req: Request) {
	...
	try {
		await prisma.avanceProduccion.delete({ where: { id: data.id } });
		return NextResponse.json(
