...
function ProduccionPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const resp = await axios.post(`/api/projects/${params.projectid}/avance`, { ...data, tarea });
      console.log(resp);
      if (resp.status === 201) {
  ...
  const handleDelete = async (index: any) => {
    try {
      ...
      const resp = await axios.delete(`/api/projects/${params.projectid}/avance`, { data: { id: index } });
      if (resp.status === 200) {
        router.refresh();
  ...
  useEffect(() => {
    ...
    async function loadAvance() {
        try {
            const response = await axios.get(`/api/projects/${params.projectid}/avance`);
